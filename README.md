...
与估计变点（CUSUM 报警时取最后归零后的本轮连续累计起点，未报警取最强非零累计
轮起点；EWMA 报警时取 q 最后实际过零——落入稳态零带 |q|≤0.1·σ_稳态
或异号——之后的同号段起点，未报警取累积标准化残差极值之后第一批；
...
s_i·(x̄_i,末批 − x̄_i,首批)，占比按各贡献**绝对值之和**归一（贡献
可正可负、互相抵消，不能用带符号合计做分母）。部分批次未引用方案时
按零量具系统误差处理并给出口径不一致告警。
